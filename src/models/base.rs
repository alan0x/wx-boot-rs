...
pub static USER_FILTER_FIELDS: Lazy<Vec<String>> = Lazy::new(|| {
    vec!["id", "ident_name", "display_name", "in_kernel"]
        .into_iter()
        .map(String::from)
        .collect()
});
...
pub struct User {
    ...
    pub contribute: Option<i64>,
    pub enable_ranking: Option<bool>,
    pub latest_export: Option<DateTime<Utc>>,
...
pub struct NewUser<'a> {
    ...
    pub in_kernel: bool,
    pub is_verified: bool,
    pub weixin_openid: Option<&'a str>,
    pub profile: Value,
    pub contribute: Option<i64>,
    pub enable_ranking: Option<bool>,
    pub updated_by: Option<i64>,
...
pub static USER_LAST_LOGIN_FILTER_FIELDS: Lazy<Vec<String>> = Lazy::new(|| {
    vec!["id", "user_id"]
        .into_iter()
        .map(String::from)
        .collect()
});
pub static USER_LAST_LOGIN_JOINED_OPTIONS: Lazy<Vec<JoinedOption>> = Lazy::new(Vec::new);
#[derive(Identifiable, Queryable, Serialize, Deserialize, Clone, Debug)]
#[diesel(table_name = user_last_login)]
