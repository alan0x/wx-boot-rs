...
static PRESERVED_IDENT_NAMES: Lazy<Vec<&'static str>> = Lazy::new(|| {
    vec![
        "admin", "administrator", "common", "guest", "client", "share", "shared",
        "share_link", "sharelink", "root", "password", "super", "realm", "creative",
        "batch", "interflow", "trade", "font", "role", "user", "deploy", "campaign",
        "auth", "oauth", "order", "output", "account", "team", "product", "variant",
        "bundle", "troupe",
    ]
});

pub const SYSTEM_LABEL_OWNER_ID: i64 = 1;
pub const UNCLASSIFIED_LABEL_NAME: &str = "未分类";
...
pub fn database_conns() -> u32 {
    env::var("DATABASE_CONNS")
        ...
        .parse::<u32>()
        .expect("DATABASE_CONNS must be u32")
}
...
pub fn super_auth_token() -> String {
    env::var("SUPER_AUTH_TOKEN").expect("SUPER_AUTH_TOKEN must be set")
}
pub fn cookie_domain() -> String {
    env::var("COOKIE_DOMAIN").expect("COOKIE_DOMAIN must be set")
}
pub fn wechat_mp_appid() -> String {
    env::var("WECHAT_MP_APPID").expect("WECHAT_MP_APPID must be set")
}
pub fn wechat_mp_secret() -> String {
    env::var("WECHAT_MP_SECRET").expect("WECHAT_MP_SECRET must be set")
}
...
pub fn generate_digit_code(len: usize) -> String {
    String::from_utf8_lossy(&rand_bytes(&DIGIT_BYTES, len)).to_owned().to_string()
}
pub fn generate_token(len: usize) -> String {
    String::from_utf8_lossy(&rand_bytes(&CHAR_BYTES, len)).to_owned().to_string()
}
pub fn generate_url_safe_token(len: usize) -> String {
    String::from_utf8_lossy(&rand_bytes(&URL_SAFE_CHAR_BYTES, len)).to_owned().to_string()
}
...
pub fn generate_password(len: usize) -> String {
    String::from_utf8_lossy(&rand_bytes(&PASSWORD_SAFE_CHAR_BYTES, len)).to_owned().to_string()
}
...
pub fn default_underscore() -> String { "_".into() }
...
pub fn add_url_list_query(query: impl QueryDsl) -> impl QueryDsl { query }
