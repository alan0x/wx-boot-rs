...
pub async fn send_local_file_with_fallbacks<T>(
    key: impl AsRef<str>,
...
{
    let key = key.as_ref();
    if fallbacks.is_empty() || local_file_exists(key).await.unwrap_or(false) {
        send_local_file(key, req_headers, res, attached_name).await;
        return;
    } else if let Some((base_path, _)) = key.rsplit_once('/') {
        for fb in fallbacks {
            let fb_key = join_path!(base_path, fb.as_ref());
            if local_file_exists(&fb_key).await.unwrap_or(false) {
                send_local_file(&fb_key, req_headers, res, attached_name).await;
                return;
            }
        }
    }
...
pub async fn local_file_exists(key: impl AsRef<str>) -> AppResult<bool> {
